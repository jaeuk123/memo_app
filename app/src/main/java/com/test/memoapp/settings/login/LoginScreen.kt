...
fun LoginScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.eventFlow.collect { event ->
            when (event) {
                LoginEvent.NavigateToHome -> navBackPress.invoke()
    ...
    Scaffold(topBar = { TitleTopBar(navBackPress, "") }) { paddingValues ->
        Column(
            ...
            Button(
                onClick = {
                    println(" 로그인 버튼 클릭")
                    loginEvent.invoke(email, password)
                },
//                enabled = uiState !is AuthUiState.Loading // 로딩 중에는 버튼 비활성화
            ) {
                Text("로그인")
            }
        }
